    ...
    */
    ...
    TaskGraph.prototype._visitGraphNode = function(
        ...
        _.forEach(task.waitingOn, function(value, dep) {
            var nonTerminalOnStates = [];
            if (dep === "either") {
                _.forEach(task.waitingOn.dep, function(eitherValue, eitherDep) {
                    nonTerminalOnStates = self._visitOneGraphNode(
                        self.tasks.either, eitherValue, eitherDep);
                    return self._visitGraphNode(
                        eitherDep, task.dep.injectableName, markers, nonTerminalOnStates);
                });
            }
            else {
    ...
    TaskGraph.prototype._populateTaskData = function() {
        ...
        return Promise.map(self.definition.tasks, function(taskData) {
            ...
            _.forEach(_.keys(taskData.waitOn), function(waitOnTask) {
                if (waitOnTask === "either") {
                    _.forEach(_.keys(taskData.waitOn[waitOnTask]), function(eitherTask){
                      self._processOneWaitOnTask(taskData.waitOn[waitOnTask], idMap, eitherTask);
                    });
                }
                else {
