...
function RestJobFactory(BaseJob, Logger, _, assert, Promise, util, HttpTool){
    ...
    */
    ...
    RestJob.prototype._run = function run(){
        var self = this;
        logger.info("Runnging A Rest Call");
        self.restClient.setupRequest(self.options)
       .catch(function(err){
            logger.error("Found error during setting up request.", {error: err});
            self._done(err);
        })
        .then(self.restClient.runRequest()
            .catch(function(err){
                logger.error('Found error on running request.', {error: err});
                self._done(err);
            })            
            .then(function(data){
